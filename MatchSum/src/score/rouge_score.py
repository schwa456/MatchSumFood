...
class WhitespaceTokenizer:
    def morphs(self, text):
        return text.strip().split() # 공백 기준으로 토큰 분리
...
class RougeScorer(scoring.BaseScorer):
    ...
    def __init__(
            self,
            rouge_types,
            split_summaries=False, # 문장을 nltk로 분리할지, \n으로 분리할지
            tokenizer=None
    ):
        ...
        if tokenizer:
            ...
            logging.info("Using whitespace tokenizer for ROUGE.")
    ...
    def score_multi(self, targets, prediction):
        ...
        for k in self.rouge_types:
            index = np.argmax([s[k].fmeasure for s in score_dicts]) # 여러 정답 중 가장 높은 f-measure를 갖는 score 선택
